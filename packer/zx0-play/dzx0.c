...
static int read_interlaced_elias_gamma() {
    ...
    /* also encode like this XXX TODO, easier to understand */
    while (!read_bit()) {
        if (bit_size >= 8) {
            value2 = value2 << 1 | read_bit();
        } else {
            value1 = value1 << 1 | read_bit();
        }
        bit_size++;
        if (bit_size == 8) value2 = 1;
    }
    value = (value2 << 8) | (value1 & 0xff);
    return value;
}
...
static void decompress() {
    ...
    /* XXX TODO support inplace and cbm style files */
